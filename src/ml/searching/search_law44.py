...
def searching_bm25_law44(request, top_n=5):
    '''
    ...
    '''
    with open(get_path("laws44.json"), 'r', encoding='utf-8') as f:
        laws44 = json.load(f)
    combined_articles = laws44["law_combined_text"]
    ...
    for index in sorted_doc_indices:
        law_i = {
            "law_name": f'ФЗ-44: {laws44["law_name"][index]}',
            "law_text": laws44["law_text"][index],
            "law_lemma_text": laws44["law_lemma_text"][index],
            "law_combined_text": laws44["law_combined_text"][index],
            "law_link": laws44["law_link"][index],
...
def searching_tf_idf_law44(request, top_n=5):
    '''
...
def unite_laws44(request):
    ...
    '''
    laws_bm = searching_bm25_law44(request)
    laws_tf = searching_tf_idf_law44(request)
    laws_togetger = []
